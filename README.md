...
Raw multi-currency sales logs were processed and standardized to ensure data integrity before visualization:
...
* **Missing Value Imputation:** Handled missing categorical records by assigning explicit Unknown attributes for unassigned departments and reps.
...
* **ABC Concentration Analysis:** Instantly identifies core revenue drivers (~80% of sales, Group A) vs. low-yielding segments (~5% of sales, Group C) to prioritize focus areas.
...
* **Loyalty Program Oversight:** Monitors card issuance (CARD vs. GUEST) per sales rep to highlight managers with low loyalty prompt rates during checkout.
* **Manager Benchmarking:** Identifies top-performing reps to establish sales standards, structure onboarding programs, or pair weaker performers for peer training.
